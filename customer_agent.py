...
class CustomerAgent:
    ...
    def __init__(self, start_pos, target_pos, state, map, batch):
        ...
        # 基本情報
        # =========================
        self.id = CustomerAgent._id_counter # 顧客IDを割り当て
        CustomerAgent._id_counter += 1 # 次に使用するIDへ更新
        ...
        # print(f"SimpleMover initialized: id={self.id}, state={self.state.name}, start_pos=({self.grid_x}, {self.grid_y}), target_pos=({self.target_x}, {self.target_y})")
